...
export default function Sidebar() {
  ...
  }, [pathname, setSidebarCollapsed]); // Do NOT include isSidebarCollapsed, otherwise opening it triggers a close
  ...
  return (
    <>
      <motion.aside
        ...
        className={cn(
          "flex flex-col h-full flex-shrink-0 relative",
          "max-md:absolute max-md:z-50 transition-transform duration-200",
          isSidebarCollapsed ? "max-md:-translate-x-full" : "max-md:translate-x-0"
        )}
        style={{
          background: "#0A0B10",
      ...
      >
        <div className="flex flex-col h-full overflow-hidden">
          ...
          {!isSidebarCollapsed && (
            <div className="px-3 pb-3 flex-shrink-0">
              <div className="p-3 rounded-xl flex items-start gap-3 transition-colors" style={{ background: "rgba(45,212,191,0.05)", border: "1px solid rgba(45,212,191,0.1)" }}>
                <div className="mt-0.5 text-[#2DD4BF]">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" /></svg>
                </div>
                <div className="overflow-hidden">
                  <p className="text-xs font-medium mb-0.5 whitespace-nowrap text-ellipsis" style={{ color: "#F1F5F9" }}>Daily Briefing</p>
                  <p className="text-[11px] leading-relaxed" style={{ color: "#94A3B8" }}>
                    Press <kbd className="px-1 py-0.5 rounded font-mono" style={{ background: "rgba(255,255,255,0.1)", fontSize: "10px" }}>B</kbd> to view your urgent leads.
                  </p>
                </div>
              </div>
            </div>
        ...
        <button
          ...
          className="absolute top-1/2 -translate-y-1/2 -right-[12px] w-[24px] h-[24px] hidden md:flex items-center justify-center rounded-full bg-[#12141F] border border-[rgba(255,255,255,0.1)] text-[#94A3B8] hover:bg-[#1A1D2E] hover:text-[#FFFFFF] transition-all z-50 cursor-pointer"
